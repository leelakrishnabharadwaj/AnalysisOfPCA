...
# convert RBG to YIQ
# In Image Processing there is a concept of Color space transform.
# Different Color space gives different information based on that you can develop many algorithms.
# YCBCR - Gray scale, blue and red components
...
def save_video(video_tensor):
    ...
    writer = cv2.VideoWriter("elliptical2.avi", fourcc, 30, (width, height), 1)
    for i in range(0,video_tensor.shape[0]):
...
def elliptical_bandpass_filter(data, lowcut, highcut, fs, order=5):
    omega = 0.5 * fs
    low = lowcut / omega
    high = highcut / omega
    b, a = signal.ellip(order,5,30,[low, high], btype='band',output='ba')
    y = signal.lfilter(b, a, data, axis=0)
    p,q,r,s = y.shape;
    pca = PCA()
    y = y.reshape(-1,3)
...
def reconstract_from_tensorlist(filter_tensor_list,levels=2):
...
#manify motion
def magnify_motion(video_name,low,high,levels=2,amplification=5):
    t,f=load_video(video_name)
    ...
    for i in range(levels):
        ...
        filter_tensor = elliptical_bandpass_filter(lap_video_list[i], low, high, f)
...
if __name__=="__main__":
    # magnify_color("test.mp4",0.4,3)
     magnify_motion("/content/drive/My Drive/Colab Notebooks/baby.mp4",0.4,3)
